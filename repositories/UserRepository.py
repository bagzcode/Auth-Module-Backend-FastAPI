...
class UserRepository(IUserRepository):
    def __init__(self):
        self.em = entity_manager.get_collection(
            os.environ.get("USER_COLLECTION"))
        self.activeSessionsEntityManager = entity_manager.get_collection(
            os.environ.get("ACTIVE_SESSIONS_COLLECTION"))
    ...
    def create_access_token(self, data: dict, expires_delta):
        ...
        encoded_jwt = jwt.encode(to_encode, os.environ.get(
            "SECRET_KEY"), algorithm=os.environ.get("ALGORITHM"))
        return encoded_jwt
    ...
    def register_token_in_session(self, token: str):
        try:
            ...
            self.activeSessionsEntityManager.insert_one(
                new_active_session.dict())
        except PyJWTError:
            raise HTTPException(
                status_code=401, detail="Authentication failed, invalid or expired token.")
    ...
    def is_session_active(self, username: str):
        ...
        existing_session = self.activeSessionsEntityManager.find_one(
            {"username": username})
        ...
        try:
            payload = jwt.decode(existing_session["access_token"], os.environ.get(
                "SECRET_KEY"), algorithms=[os.environ.get("ALGORITHM")])
            expiry_time = payload.get("exp")
